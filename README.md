<div align="center">

# Hey, I'm Babar Haneef Zargar 👋

### CS50P Certified · Aspiring ML Engineer · Based in Kashmir, India 🏔️

[![LinkedIn](https://img.shields.io/badge/LinkedIn-0077B5?style=flat-square&logo=linkedin)](https://linkedin.com/in/babarzargar95)
[![GitHub](https://img.shields.io/badge/GitHub-181717?style=flat-square&logo=github)](https://github.com/BabarZargar)

</div>

## 🚀 What I'm Currently Doing

- 📖 Learning **NumPy, Pandas, and Matplotlib** — the foundation of every ML engineer
- ⏳ Waiting on financial aid approval for **Andrew Ng's ML Specialization** on Coursera
- 🗃️ Building and maintaining **Stash** — my CS50P final project

---

## 🏆 Certifications

| Certificate | Institution | Year |
|-------------|-------------|------|
| CS50's Introduction to Programming with Python | Harvard University | 2026 |
| Machine Learning Specialization *(in progress)* | Coursera / DeepLearning.AI | 2026 |

🔗 [Verify CS50P Certificate](https://cs50.harvard.edu/certificates/9131cc52-4ee6-4c97-b7d3-ed8f8ccb7a89)

---

## 🛠️ Skills & Tools

**Languages**

![Python](https://img.shields.io/badge/Python-3776AB?style=flat-square&logo=python&logoColor=white)
![C](https://img.shields.io/badge/C-00599C?style=flat-square&logo=c&logoColor=white)

**Data Science Stack**

![NumPy](https://img.shields.io/badge/NumPy-013243?style=flat-square&logo=numpy)
![Pandas](https://img.shields.io/badge/Pandas-150458?style=flat-square&logo=pandas)
![Matplotlib](https://img.shields.io/badge/Matplotlib-11557C?style=flat-square)
![Jupyter](https://img.shields.io/badge/Jupyter-F37626?style=flat-square&logo=jupyter)

**Tools**

![Git](https://img.shields.io/badge/Git-F05032?style=flat-square&logo=git&logoColor=white)
![GitHub](https://img.shields.io/badge/GitHub-181717?style=flat-square&logo=github)
![VS Code](https://img.shields.io/badge/VS%20Code-007ACC?style=flat-square&logo=visual-studio-code)
![Pytest](https://img.shields.io/badge/pytest-0A9EDC?style=flat-square&logo=pytest&logoColor=white)

---

## 📌 Featured Projects

### 🗃️ Stash — Your Second Brain
> A CLI tool that automatically categorizes and stores anything you want to remember — links, meetings, passwords, ideas — using regex pattern matching and keyword scoring.

**Built with:** Python · Regex · JSON · pytest · pytz
**Tests:** 24 pytest tests, all passing

🔗 [View Repository](#) · 📺 [Watch Demo](https://youtu.be/qaldgBsRIxU?si=-hT-RK27n_7FgmRs)

---

### 🧠 ML Journey
> My complete learning journey from Python libraries to Machine Learning — every notebook, every concept, documented as I go.

**Currently covering:** NumPy · Pandas · Matplotlib · ML Specialization *(coming soon)*

🔗 [View Repository](#)

---

## 📊 GitHub Stats

<div align="center">

![Babar's GitHub Stats](https://github-readme-stats.vercel.app/api?username=BabarZargar&show_icons=true&theme=dark&hide_border=true)

![Top Languages](https://github-readme-stats.vercel.app/api/top-langs/?username=BabarZargar&layout=compact&theme=dark&hide_border=true)

</div>



<div align="center">

*"An investment in knowledge pays the best interest."*

⭐ Feel free to explore my repositories and reach out!

</div>
EOF
echo "Done"
<img width="95%" src="https://github-readme-activity-graph.vercel.app/graph?username=BabarZargar&theme=tokyo-night&hide_border=true&bg_color=000000"/>

</div>

---

<div align="center">

<img src="https://capsule-render.vercel.app/api?type=waving&color=0:8B0000,100:0d0d0d&height=120&section=footer"/>

</div>




